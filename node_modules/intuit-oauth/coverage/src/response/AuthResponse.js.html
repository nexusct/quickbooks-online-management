...
<head>
    <title>Code coverage report for src/response/AuthResponse.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../../index.html">All files</a> / <a href="index.html">src/response</a> AuthResponse.js</h1>
        <div class='clearfix'>
...
&nbsp;
    if (response.data) {
      ...
      this.body = typeof response.data === 'string' ? <span class="branch-0 cbranch-no" title="branch not covered" >response.data </span>: JSON.stringify(response.data);
      
      // Handle QuickBooks API error response
      <span class="missing-if-branch" title="if path not taken" >I</span>if (response.data.Fault) {
<span class="cstat-no" title="statement not covered" >        this.json = {</span>
...
<span class="cstat-no" title="statement not covered" >        this.response = {</span>
          ...response,
          data: this.json,
      ...
      try {
        const parsedBody = typeof response.body === 'string' ? JSON.parse(response.body) : <span class="branch-1 cbranch-no" title="branch not covered" >response.body;</span>
        
        // Handle QuickBooks API error response
        <span class="missing-if-branch" title="if path not taken" >I</span>if (parsedBody.Fault) {
<span class="cstat-no" title="statement not covered" >          this.json = {</span>
...
AuthResponse.prototype.getJson = function getJson() {
  ...
  <span class="missing-if-branch" title="if path not taken" >I</span>if (this.json !== null) {
<span class="cstat-no" title="statement not covered" >    return this.json;</span>
  }
&nbsp;
  // Try to parse the body if we have one
  if (this.body) {
    try {
      this.json = typeof this.body === 'string' ? JSON.parse(this.body) : <span class="branch-1 cbranch-no" title="branch not covered" >this.body;</span>
      
      // Handle QuickBooks API error response
      <span class="missing-if-branch" title="if path not taken" >I</span>if (this.json.Fault) {
<span class="cstat-no" title="statement not covered" >        this.json = {</span>
 ...
 */
AuthResponse.prototype.isContentType = function <span class="fstat-no" title="function not covered" >isContentType(</span>contentType) {
<span class="cstat-no" title="statement not covered" >  return this.getContentType().indexOf(contentType) &gt; -1;</span>
};
&nbsp;
/**
 * getContentType
 ...
 */
AuthResponse.prototype.isJson = function <span class="fstat-no" title="function not covered" >isJson(</span>) {
<span class="cstat-no" title="statement not covered" >  return this.isContentType('application/json');</span>
};
&nbsp;
